...
The Duo Workflow runner is a lightweight container, so these runs are scoped as pre-merge validation, not production-scale load simulations. The demo apps were built to fit the hackathon's runner constraints while proving the concept end-to-end across three stacks. The value at the MR stage is catching the bugs that unit tests miss: latency regressions, thread-safety failures, and degraded throughput under concurrent load, targeted at the code that just changed. The architecture itself is runner-agnostic: the agent logic (diff → GraphRAG → k6 script → execute → report) doesn't depend on the container. A runner with Docker Compose or a staging environment would handle full-stack applications with external databases, and the per-project `AGENTS.md` config already abstracts the execution command per app. Fully automated.
